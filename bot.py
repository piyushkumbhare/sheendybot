...
@bot.tree.command(name="start-reaction", description="Re-syncs emojis and starts Shindi behavior")
        ...
        if(len(match) > 0):
            ...
            for group in match:    
                ...
                for i in emoji_names:
                    if(i.name == group):
                        print(f"Before: {text}")
                        text = text.replace(f":{i.name}:", f"<a:{i.name}:{i.id}>")
                        print(f"After: {text}")
                        valid = True                        
            if(valid):
                await message.delete()
                avatar_bytes = await user.display_avatar.read()
                webhook = await message.channel.create_webhook(name= user.display_name if (user.nick == None) else user.nick, avatar=avatar_bytes)
